...
- **F-ANP-47 公開済み記事の有料化（運営者指示「有料化機能作って」）**:
  ...
  - **実績 (2026-09-25)**: 公開済みの無料記事 **9 本を有料化**（¥500〜¥780）。KYC は完了済みで
    `kyc_required` には当たらなかった。価格提案が無い記事（企画時から無料のもの）は回遊用に
    無料のまま残した。
  - **副産物: 有料記事の新規公開 (F-ANP-16b) が一度も成功していなかった原因も判明・修正**。
    公開設定で「有料」を選ぶと主ボタンが「投稿する」ではなく「有料エリア設定」に変わり、
    さらにその画面で「ラインをこの場所に変更」を押さないと投稿が無反応になる。加えて有料公開は
    URL 遷移も完了モーダルも出ないため、公開できていても `blocked` と記録されていた
    （公開 API `GET /api/v3/notes/<id>` の `status` を確認手段に追加）。
    これで **あや＠AI副業の ¥1,480 記事を有料で新規公開できた**。
  - note は公開済み記事のエディタ編集を**オートセーブ**するが、**公開中の本文は「更新する」を
    押すまで変わらない**（ドライラン後に公開 API が `price=0` のままであることを確認済み）。
  - note の `/settings/fee`（お支払い口座＝KYC 状態）はパスワード再入力を要求するようになったため、
    外部から KYC 済みかを読めない。`kyc_required` の発生で判断する。
  - 公開に失敗したまま DB に `published` が刻まれる取りこぼしが 1 件あった。
    `GET /api/v3/notes/<noteId>` で全公開記事の実在を突き合わせると早い。
...
### 有料記事の自動公開 / アイキャッチの画風バリエーション（2026-09-24）
...
### 記事の方針の区分化（F-ANP-07b, 2026-09-22）
- 想定読者/トーンを横幅いっぱいに、方針を 6 区分（主なテーマ/記事のフォーマット/文末表現・禁止事項/CTA/品質判定項目/その他）の
  テキストボックスへ。保存形式は従来の 1 本テキスト（【見出し】付き連結）なので互換。既存の長文は初回表示時にラベル語で自動振り分け
  （気に入らなければ「AI で作り直す」で 6 区分に再生成）。judge は【品質判定項目】を減点基準として明示的に使う。
