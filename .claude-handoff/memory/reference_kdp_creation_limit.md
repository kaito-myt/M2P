...
**追記 (2026-09-18)**: 本棚を全頁スキャン(87 listing、`kdp-scan-bookshelf.mjs` をページ送り対応に修正)したところ
**審査中(レビュー中)・下書きは 0 件、全件販売中**なのに、サーバー dispatcher(毎日 11:07 JST)もローカル日次も
`creation_limit` で止まり続けている。→「審査滞留に応じた制限」ではなく**新規タイトル作成数の期間上限(ローリング)**の可能性が高い。
KDP 側の制限なのでコードでは解消不可。DB の「submitted」20 冊のうち 9 冊は実際は販売中(同期漏れ)、11 冊は
KDP 上に存在しない幽霊(ローカル出版ツールが submitted にしたが実作成されず) → unlisted に戻してキューへ再投入した。
本棚同期 `scripts/kdp-sync-shelf.sh` は同題別レコードの ASIN 衝突(books_asin_key)を回避するよう修正済み。
