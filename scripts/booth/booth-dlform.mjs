...
// 「ダウンロード」商品を登録 をクリック
await page.getByText('「ダウンロード」商品を登録',{exact:false}).first().click({force:true}).catch(e=>console.log('click err',e.message.slice(0,40)));
await page.waitForTimeout(7000);
console.log('DLフォームURL:',page.url());
const info=await page.evaluate(()=>{
  const vis=e=>e.offsetParent!==null;
  const fields=[...document.querySelectorAll('input,textarea,select')].filter(vis).map(e=>`${e.tagName.toLowerCase()}[name=${e.name||''}][type=${e.type||''}][id=${e.id||''}]${e.placeholder?' ph='+e.placeholder.slice(0,14):''}`).slice(0,40);
  const files=[...document.querySelectorAll('input[type=file]')].map(f=>({name:f.name,accept:(f.accept||'').slice(0,40),id:f.id}));
  const labels=[...document.querySelectorAll('label,h2,h3,legend')].filter(vis).map(l=>(l.textContent||'').replace(/\s+/g,' ').trim().slice(0,28)).filter(Boolean).slice(0,35);
...
await page.screenshot({path:path.join(REPO,'scripts/booth/out/dlform.png'),fullPage:true}).catch(()=>{});
