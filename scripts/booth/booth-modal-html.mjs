...
const html = await page.evaluate(() => {
  const m = document.querySelector('.booth-modal-content-wrapper') || document.querySelector('.booth-modal');
  return m ? m.innerHTML : '(none)';
});
// 整形して主要要素を抽出
console.log('=== モーダルHTML(タグ構造) ===');
console.log(html.replace(/></g, '>\n<').replace(/\s+class=/g, ' class=').slice(0, 2500));
await browser.close();
process.exit(0);
