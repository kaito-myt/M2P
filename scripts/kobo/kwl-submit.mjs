...
const browser = await chromium.launch({ headless: true, args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', '--disable-blink-features=AutomationControlled'] });
...
// --- テキスト欄(name属性で特定) ---
const fill = async (name, v) => { if (v == null || v === '') return; const e = await page.$(`[name="${name}"]`); if (e) { await e.click().catch(() => {}); await e.fill(String(v)).catch(() => {}); } };
...
const genreLeaf = (() => {
  ...
  if (/money|invest|business|side_business|finance|経済|投資|副業|ビジネス/.test(g)) return 'ビジネス・経済・就職一般';
  if (/self_help|自己啓発|人文|思想/.test(g)) return '人文・思想・社会一般';
  if (/health|beauty|sport|hobby|美容|健康|趣味/.test(g)) return 'ホビー・スポーツ・美術一般';
  if (/novel|fiction|essay|literature|小説|エッセイ|文芸/.test(g)) return '小説・エッセイ一般';
  if (/light_novel|ラノベ/.test(g)) return 'エンターテインメント一般';
  return 'ノンフィクション一般';
})();
...
// --- ジャンル(必須・1つ以上): アップロードの再描画後に葉をクリック ---
{
  const esc = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  let picked = await page.locator('li,a,button,[role=button],span').filter({ hasText: new RegExp('^' + esc(genreLeaf) + '$') }).first().click({ force: true, timeout: 8000 }).then(() => genreLeaf).catch(() => null);
  if (!picked) picked = await page.locator('li,a,button,[role=button],span').filter({ hasText: /^ノンフィクション一般$/ }).first().click({ force: true, timeout: 6000 }).then(() => 'ノンフィクション一般').catch(() => null);
  await page.waitForTimeout(1500);
  const stillRed = await page.evaluate(() => [...document.querySelectorAll('*')].some((e) => /1つ以上選択ください/.test(e.textContent || '') && e.offsetParent !== null));
  console.log('ジャンル選択:', picked, '必須警告残:', stillRed);
}
await shot('filled');

// --- 価格(JPY) ---
await fill('prices[0]', String(priceJpy));
await page.waitForTimeout(1500);
console.log('価格入力: ¥' + priceJpy);
...
if (SUBMIT) {
  ...
  await page.locator('button').filter({ hasText: /^出版する$/ }).first().click({ force: true, noWaitAfter: true, timeout: 10000 }).then(() => console.log('出版click')).catch((e) => console.log('出版click err', e.message.slice(0, 50)));
  await page.waitForTimeout(8000);
  // 確認モーダル
  const modal = page.locator('[role=dialog] button, .modal button').filter({ hasText: /出版|確定|はい|OK|続行/ });
  if (await modal.count().catch(() => 0)) { await modal.first().click({ force: true, noWaitAfter: true }).catch(() => {}); await page.waitForTimeout(5000); }
  await shot('submitted');
  console.log('全POST:', JSON.stringify(responses.slice(-10)));
