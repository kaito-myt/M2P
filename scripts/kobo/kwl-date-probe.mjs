...
// 日付セクションの構造を精密ダンプ: 「発売予定日」「発売日」ラベルとその配下の入力
const dates=await page.evaluate(()=>{
  const out=[];
  for(const lbl of [...document.querySelectorAll('label,legend,h3,h4,p,span')].filter(e=>/発売予定日|発売日|公開日|出版日/.test((e.textContent||'').trim())&&(e.textContent||'').length<30)){
    const sec=lbl.closest('div,section,fieldset')?.parentElement||lbl.parentElement;
    const fields=[...sec.querySelectorAll('input,select,[role=spinbutton],[contenteditable]')].filter(e=>e.offsetParent!==null).map(e=>({tag:e.tagName,name:e.getAttribute('name')||'',al:e.getAttribute('aria-label')||'',ph:e.placeholder||'',val:e.value||e.textContent||'',role:e.getAttribute('role')||''})).slice(0,6);
    out.push({label:(lbl.textContent||'').replace(/\s+/g,' ').trim().slice(0,20),fields});
  }
  return out.slice(0,4);
});
console.log('日付セクション:',JSON.stringify(dates,null,1));
