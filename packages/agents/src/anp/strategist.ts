...
export function buildEditorialUserMessage(input: NoteAccountProfileInput, existingPolicy?: string): string {
  const lines = [
    ...
    '- sections: 記事の方針・トンマナを 7 区分に分けたオブジェクト。各区分はプレーンテキストの箇条書き (「・」始まり) で、',
    ...
    '  seo (SEO対策: note 内・検索エンジンで狙うキーワード領域 (3 語以上のロングテール)、タイトルの付け方、',
    '  よく使うハッシュタグ 5 個、避ける語、1 記事 1 テーマ等の決まり、1500 字以内)、',
    '  other (その他: 引用/出典の扱い・画像・免責など上記に入らないもの、2000 字以内)。',
    '- rationale: なぜこの方針か 2〜3 行 (任意)。',
    '',
    '出力形式: {"target_reader": "...", "tone": "...", "sections": {"themes": "...", "format": "...", "style_rules": "...", "cta": "...", "quality": "...", "seo": "...", "other": "..."}, "rationale": "..."} の JSON のみ。',
