...
describe('buildAnpPromptSeeds', () => {
  it('5 role すべてに genre=null v1 active 行を1本ずつ生成する', () => {
...
describe('buildAnpModelAssignmentSeeds', () => {
  it('5 role すべてに anthropic モデルを割当てる', () => {
...
describe('runSeedAnp', () => {
  it('prompt / modelAssignment を upsert し、2 回実行しても追加 create が発生しない (idempotent)', async () => {
    ...
    expect(first.prompts).toBe(5);
    expect(first.modelAssignments).toBe(5);
    expect(prisma.prompt.create).toHaveBeenCalledTimes(5);
    expect(prisma.modelAssignment.create).toHaveBeenCalledTimes(5);

    await runSeedAnp(prisma as never, silentLogger);
    expect(prisma.prompt.create).toHaveBeenCalledTimes(5);
    expect(prisma.modelAssignment.create).toHaveBeenCalledTimes(5);
    expect(prisma.prompt.update).toHaveBeenCalledTimes(5);
    expect(prisma.modelAssignment.update).toHaveBeenCalledTimes(5);
  });
});
