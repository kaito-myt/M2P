...
- **F-ANP-47 公開済み記事の有料化（2026-09-25）**: 運営者指示「有料化機能作って」。F-ANP-45 で
  ...
  - 実装メモ: `insertPaywallMarker(page, pressEnter)` の第 2 引数を追加した（執筆時は改行して
    続きを打つが、既存本文への後付けでは改行すると空段落が残る）。
  - **実機デバッグで判明した note の仕様（2026-09-25、`scripts/anp/note-paywall-recon.mjs` /
    `note-paid-publish-recon.mjs` で採取）**:
    1. 有料エリアは本文中の **`<paywall-line>` というカスタム要素**。既存有無の判定はこの
       要素を見る。本文テキスト（「ここから先は」等）で判定すると記事の言い回しに誤反応する
       （実際に誤反応し、有料ラインを挿さずに公開設定へ進んでいた）。
    2. ブロックのクリックは **Playwright の locator クリック**で行う。`evaluate` 内で
       `scrollIntoView` した直後の座標は viewport 外のことがあり（実測 y=1683 / 高さ1300）、
       `page.mouse.click` が空振りしてカーソルが本文先頭のまま＝**記事の冒頭に有料ラインが
       入る事故**になる。挿入後は `<paywall-line>` の位置を数え、無料部分が全体の 5% 未満なら
       中断する。
    3. **note エディタのブロックと Markdown の段落は 1:1 対応しない**（複数段落が 1 つの
       `<p>` に束ねられる）。そのため `computePaywallSplit` は**見出しを優先して境界に選び**、
       `anchorText`（見出し文）でエディタ側のブロックをテキスト一致で特定する（index は
       フォールバック）。実測では H3 の直後に `<paywall-line>` が入り、見出しだけ無料で見えて
       中身が有料という自然な形になる。
    4. **有料を選ぶと公開設定画面の主ボタンが「更新する」/「投稿する」ではなく
       「有料エリア設定」になる**。その先の画面に「更新する」と「ラインをこの場所に変更」
       （候補位置ごと）が並ぶ。実装は 確定ボタン候補 → 無ければ「有料エリア設定」→
       再度確定ボタン候補、の順で押す。
       **これは `pipeline.note.publish` の有料公開 (F-ANP-16b) が一度も成功していなかった
       原因でもある**（「投稿する」しか探しておらず必ず `blocked` になっていた）。
       同じ修正を `publishOne` にも入れた。
    5. **有料エリア設定画面でラインを選ばないと「投稿する」が無反応**。本文に
       `<paywall-line>` が無い状態だと、この画面はブロック間に「ラインをこの場所に変更」
       ボタンを並べるだけで、どれかを押すまで投稿が進まない（押しても画面が変わらない）。
       `ensurePaidAreaLine(page, freeBlockCount)` が無料ブロック数に対応するボタンを押して
       ラインを確定してから投稿/更新する。**執筆時の本文へのマーカー挿入が効かなかった記事でも
       この経路で有料公開できる**（実測: 21 個のボタンのうち 16 番目を押して ¥1,480 で公開成功）。
    6. **有料公開は URL 遷移も「記事が公開されました」モーダルも出ない**ため、
       `waitForPublishConfirmation` が公開できているのに `blocked` を返していた。
       公開 API (`GET /api/v3/notes/<noteId>` の `status==='published'`) も確認手段に加えた。
  ...
  - 2026-09-25 実績: **公開済みの無料記事 9 本を有料化**（¥500〜¥780、所要 ~35 秒/本。
    ...
    （あや＠AI副業「ChatGPT×Kindle出版」完全ロードマップ ¥1,480）。
  - DB が `published` なのに note 上に存在しない記事が 1 件あった（公開に失敗したまま
    `published` が刻まれていた）。`status='ready'` に戻して有料で出し直した。
    公開済み記事の実在確認は `GET /api/v3/notes/<noteId>` が手軽（`note.publish.status.sync`
    が拾えていなかったケース）。
