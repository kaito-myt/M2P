...
**【2026-09-25 実地調査】なぜ「出版が止まって」いたのか** — 運営者報告「作成済みの本の出版も止まってない？」の原因:

1. **Amazon 側**: CREATE のたびに 「**本の作成数制限を超えました / このアカウントで提出可能な本の数を超えています**」
   モーダルが出ている (R2 の `debug/kdp-submit/*-blocked1-*.png` で 9/21・9/22・9/23・9/24 いずれも同じ画面を確認)。
   本棚に **Kindle の下書きは 0 件** で、上書きできる下書きが無いため必ず CREATE になる。
2. **こちら側の設計欠陥 (これが「1 冊も出ない」を決定づけていた)**: 制限を検知すると
   `kdp_creation_paused_until = 翌 JST 0 時` で **全体を停止**していた。停止が明ける JST 0:07 に 1 冊試し、
   また制限 → 翌日まで停止、の繰り返しで **1 日 1 回しか試行しない**。Amazon の枠がいつ空くかは外から
...
- **F-097b 販売状態の同期 (`paperback.status.sync`, cron 日次 07:40 JST)**: アシスト実行は「投稿できた」時点で
  ...
  `<種別> <状態> 提出日: … ¥<価格> … ASIN: <ASIN>` というテキストを持つ。**Kindle の ASIN で検索すると
  そのカードだけが出る**ので、同カード内のペーパーバック行から状態と紙版 ASIN (`books.pb_asin`) を読む。
  販売中→`published`(キューから降ろす) / レビュー中・出版準備中→`submitted` / 下書き→`drafted` /
  販売停止中・ブロック→`failed`。**行が無い場合 (not_found) は検索の取りこぼしと区別できないので触らない**。
  セッション切れを検知したら即座に打ち切る (再ログインは別タスクの責務)。
