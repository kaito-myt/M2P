...
| F-090 | **Amazon Ads 広告費＋パフォーマンス取込（2026-08-26 広告費計上→2026-09-21 パフォーマンス拡張）** — 運営者が Amazon Ads API の利用承認を取得。(a) **広告費計上(2026-08-26)**: worker `ads.spend.fetch`（日次 cron 04:00 JST）が公式 Amazon Advertising API v3 (Reporting) から Sponsored Products の日次費用/売上/クリック/インプレッションを取得し `ad_spend` に upsert。当月コスト・純利益・ROAS に自動算入(`cost-meter-core.ts`)。creds(`AMAZON_ADS_*` env)未設定なら no-op スキップ(未接続扱い)。(b) **パフォーマンス取込拡張(2026-09-21)**: 運営者要望「広告パフォーマンス(キャンペーン別・書籍別)とコストを見られるように」。同タスクを拡張し、①`spCampaigns`(campaign 粒度)からキャンペーン別内訳を `ad_campaign_stats` へ、②`spAdvertisedProduct`(ASIN 粒度)から書籍別内訳を `ad_product_stats` へ upsert、③`POST /sp/campaigns/list` でキャンペーンの名前/状態/日予算を取得し各キャンペーンの最新日行に反映、④SB/SD(`sbCampaigns`/`sdCampaigns`)は権限が無い場合があるため best-effort(失敗は warn で skip)、⑤通貨が JPY 以外の場合は `app_settings.latest_fx_rate` で円換算、⑥Reporting API の DAILY 1リクエスト最大31日制限に対応する期間分割。新画面 **`/ads`(S-030)** で接続状態(最終取得日時＋「今すぐ取得」)、当月 KPI(広告費/広告経由売上/ROAS/ACOS/インプレッション/クリック/CTR/CPC/注文数、前月比)、日次トレンド(広告費 vs 広告経由売上)、キャンペーン別一覧(ROAS降順)、書籍別一覧(広告費 vs 当月印税)を表示。期間切替は当月/先月/直近30日。接続用 OAuth ヘルパー `scripts/ads/amazon-ads-oauth.mjs`(運営者がローカルで1回実行し、認可コード交換→プロファイル一覧取得→Railway設定用の5 env を表示)。受入基準: creds 未接続時は `/ads` に接続手順(OAuth 実行方法)を案内、接続後は日次 cron または「今すぐ取得」で `ad_spend`/`ad_campaign_stats`/`ad_product_stats` が更新され、書籍別テーブルで広告費と印税の差が確認できる。 | S-030 | P1 | Phase 2 |
